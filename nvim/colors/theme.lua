...
hi('TabLineFill',  { fg = colors.fg, bg = colors.bg })                              -- The remaining space and background
...
-- Diff (Vimdiff: nvim -d file1 file2)
hi('DiffAdd',    { fg = colors.green })                     -- Lines that are added
hi('DiffChange', { fg = colors.grey })                      -- Lines that are changed
hi('DiffDelete', { fg = colors.red })                       -- Lines that are removed (dashed line)
hi('DiffText',   { bg = colors.greyblue })                  -- Changed text inside a changed line
...
-- Spelling (:set spell)
hi('SpellBad',   { undercurl = true, sp = colors.errorred })    -- Words spelt wrong
...
hi('@lsp.type.property',      { fg = colors.fg })           -- object.property, struct fields
hi('@lsp.type.parameter',     { fg = colors.parameter })    -- function(param)
...
-- LSP diagnostics virtual text (inline)
...
-- LSP signature help
hi('LspSignatureActiveParameter', { fg = colors.highlight, bold = true })       -- Current param in call

-- Treesitter
hi('@type',                  { fg = colors.types })                             -- int, float, char
hi('@type.builtin',          { fg = colors.types })                             -- Built-in types
hi('@function',              { fg = colors.func })                              -- Function definitions
hi('@function.builtin',      { fg = colors.func })                              -- Built-in functions (print)
hi('@function.call',         { fg = colors.func })                              -- Function calls
hi('@keyword',               { fg = colors.keyword })                           -- General keywords
hi('@keyword.return',        { fg = colors.keyword })                           -- return
hi('@keyword.function',      { fg = colors.keyword })                           -- function keyword
hi('@string',                { fg = colors.string })                            -- "strings"
hi('@number',                { fg = colors.number })                            -- 1, 2, 3
hi('@boolean',               { fg = colors.keyword })                           -- true, false
hi('@comment',               { fg = colors.comment, italic = true })            -- Comments
hi('@variable',              { fg = colors.fg })                                -- Regular variables
hi('@variable.builtin',      { fg = colors.keyword })                           -- self, this
hi('@constant',              { fg = colors.types })                             -- NULL, EXIT_SUCCESS
hi('@constant.builtin',      { fg = colors.keyword })                           -- Built-in constants
hi('@parameter',             { fg = colors.parameter })                         -- Function parameters
hi('@constructor',           { fg = colors.class })                             -- Constructor calls
hi('@constructor.lua',       { fg = colors.delimiter })                         -- {} table constructors in Lua (override, not class colour)
hi('@punctuation.bracket',   { fg = colors.delimiter })                         -- () [] {}
hi('@punctuation.delimiter', { fg = colors.delimiter })                         -- , ; .
hi('@operator',              { fg = colors.operator })                          -- + - = *

-- RainbowDelimiter
hi('RainbowDelimiterYellow', { fg = colors.delimiter })
hi('RainbowDelimiterViolet', { fg = '#DA70D6' })
hi('RainbowDelimiterRed',    { fg = '#E06C75' })
hi('RainbowDelimiterBlue',   { fg = '#61AFEF' })
hi('RainbowDelimiterGreen',  { fg = '#3CB371' })
hi('RainbowDelimiterOrange', { fg = '#FF7F50' })
hi('RainbowDelimiterCyan',   { fg = '#56B6C2' })
